  ...
  group('WorkoutSpotModel -', () {
    group('distanceFromUserDisplay -', () {
      test('when distance from user is not calculated, returns null', () {
        final String? result = const WorkoutSpotModel().distanceFromUserDisplay;
        expect(result, null);
      });
      test('when distance from user is over 5 km, returns rounded distance without the fractional part', () {
        final String? result = const WorkoutSpotModel(distanceFromUserInKm: 5.876).distanceFromUserDisplay;
        expect(result, '6');
      });
      test('when distance from user is below 5 km, returns distance with one fractional digit', () {
        final String? result = const WorkoutSpotModel(distanceFromUserInKm: 2.8888).distanceFromUserDisplay;
        expect(result, '2.9');
      });
    });
  });
